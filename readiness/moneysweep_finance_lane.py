"""moneysweep-pr politics / public-finance intake lane (issue #114).

Consumes the PR-intake router's moneysweep-pr lane export
(``moneysweep_derivatives.csv``) and fans each derivative out into the
...
This is the moneysweep-pr counterpart to spiderweb-pr's
...
INPUT_FILENAME = "moneysweep_derivatives.csv"
LANE_ID = "moneysweep_politics_finance_lane"
# Versions the FINANCE-LANE REPORT contract only (the report dict / JSON this
# module emits, modeled by schemas/moneysweep_finance_lane_report.schema.json).
# This is the single source of truth for that version. It is INDEPENDENT of the
# federation "moneysweep-pr-export" contract, which is versioned separately in
# scripts/build_export_package.py (currently 1.2.0) — do not couple the two.
EXPORT_CONTRACT_VERSION = "1.0.0"
REPORT_FILENAME = "moneysweep_finance_lane_report.json"
...
REVIEW_TABLES = (
    ...
    "moneysweep_crosswalk_queue.csv",
)
...
def build_moneysweep_finance_lane(input_dir, output_dir=None) -> dict[str, Any]:
    """Normalize the router's moneysweep-pr derivatives into the finance lane.

    Reads ``moneysweep_derivatives.csv`` from *input_dir*, writes the
    ...
    *input_dir*), and returns the report dict.
    ...
    lines = [
        "# moneysweep-pr Politics / Public-Finance Update",
        "",
