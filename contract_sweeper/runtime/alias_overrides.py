...
DEFAULT_OVERRIDE_PATH = Path(__file__).resolve().parents[2] / "registries" / "alias_overrides.yaml"
...
def load_overrides(path: Path | None = None) -> dict[str, str]:
    ...
    if not isinstance(entries, list):
        raise AliasOverrideError(f"{target}: top-level 'entries' must be a list")

    mapping: dict[str, str] = {}
    for idx, entry in enumerate(entries):
        if not isinstance(entry, dict):
            raise AliasOverrideError(f"{target}: entry #{idx} is not a mapping")
        canonical_raw = entry.get("canonical_name")
        if not canonical_raw:
            raise AliasOverrideError(f"{target}: entry #{idx} missing canonical_name")
        canonical_norm = normalize_name(canonical_raw)
        if not canonical_norm:
            raise AliasOverrideError(f"{target}: entry #{idx} canonical_name normalizes to empty")
        aliases = entry.get("aliases") or []
        if not isinstance(aliases, list):
            raise AliasOverrideError(f"{target}: entry #{idx} aliases must be a list")
        ...
        for alias in [canonical_raw, *aliases]:
            ...
            if existing and existing != canonical_norm:
                raise AliasOverrideError(
                    f"{target}: alias '{alias}' maps to both '{existing}' and '{canonical_norm}'"
                )
...
def _check_cycles(mapping: dict[str, str]) -> None:
    ...
    for start in mapping:
        ...
        while current in mapping and mapping[current] != current:
            if current in seen:
                raise AliasOverrideError(f"alias cycle detected starting at '{start}'")
            seen.add(current)
