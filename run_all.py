...
def main() -> int:
    ...
    # ------------------------------------------------------------------
    logger.info("[Step 1/25] Setting up directories...")
    try:
        from scripts.setup_directories import main as setup_dirs
        setup_dirs(root)
        steps["dirs"] = True
        logger.info("[Step 1/25] Done.\n")
    except Exception as e:
        logger.error(f"[Step 1/25] FAILED: {e}")
        steps["dirs"] = False
        return 1
    ...
    # ------------------------------------------------------------------
    logger.info("[Step 2/25] Generating download instructions...")
    try:
        from scripts.download_instructions import main as gen_instructions
        gen_instructions(root)
        steps["instructions"] = True
        logger.info("[Step 2/25] Done.\n")
    except Exception as e:
        logger.error(f"[Step 2/25] FAILED: {e}")
    ...
    if skip_download:
        logger.info("[Step 3/25] SKIPPED (--skip-download / --manual-only)\n")
    else:
        logger.info("[Step 3/25] Auto-downloading datasets...")
        try:
            ...
            steps["download"] = True
            logger.info(f"[Step 3/25] Done ({download_count} files ready).\n")
        except ImportError:
            logger.warning("[Step 3/25] Auto-download unavailable (missing requests/lxml).")
            ...
            logger.error(f"[Step 3/25] FAILED: {e}")
            steps["download"] = False
    ...
    if args.skip_validation:
        logger.info("[Step 4/25] SKIPPED (--skip-validation)\n")
    else:
        logger.info("[Step 4/25] Validating downloaded files...")
        try:
            ...
            logger.info(f"[Step 4/25] Done (exit: {validation_result}).\n")
        except Exception as e:
            logger.error(f"[Step 4/25] FAILED: {e}")
            validation_result = 1
    ...
    if args.skip_normalize:
        logger.info("[Step 5/25] SKIPPED (--skip-normalize)\n")
    else:
        logger.info("[Step 5/25] Normalizing expansion inputs...")
        try:
            ...
            logger.info(f"[Step 5/25] Done ({normalize_count} files normalized).\n")
        except Exception as e:
            logger.error(f"[Step 5/25] FAILED: {e}")
            normalize_count = 0
    ...
    # ------------------------------------------------------------------
    if args.skip_dedup:
        logger.info("[Step 5.5/25] SKIPPED (--skip-dedup)\n")
    else:
        logger.info("[Step 5.5/25] Building deduplicated master...")
        try:
            from scripts.deduplicate_master import main as build_master
            dedup_stats = build_master(root)
            if dedup_stats["master_rows"] > 0:
                logger.info(
                    f"[Step 5.5/25] Done — {dedup_stats['master_rows']:,} rows, "
                    f"{dedup_stats['duplicates_removed']:,} cross-file dupes removed.\n"
                )
            else:
                logger.info("[Step 5.5/25] Done (no normalized files found yet).\n")
        except Exception as e:
            logger.error(f"[Step 5.5/25] FAILED: {e}")
            dedup_stats = None
    ...
    if args.skip_coverage:
        logger.info("[Step 6/25] SKIPPED (--skip-coverage)\n")
    else:
        logger.info("[Step 6/25] Validating expansion coverage...")
        try:
            from scripts.validate_expansion_coverage import main as validate_coverage
            coverage_result = validate_coverage(root)
            logger.info(f"[Step 6/25] Done (exit: {coverage_result}).\n")
        except Exception as e:
            logger.error(f"[Step 6/25] FAILED: {e}")
            coverage_result = 1
    ...
    if args.skip_enrichment:
        logger.info("[Step 7/25] SKIPPED (--skip-enrichment)\n")
    else:
        import os as _os
        ...
        if not has_key:
            logger.info("[Step 7/25] SKIPPED — SAM_API_KEY not set.")
            ...
            logger.info("[Step 7/25] SKIPPED — no master data (download files first)\n")
            enrichment_result = "SKIPPED — no master data"
        else:
            logger.info("[Step 7/25] Running SAM.gov UEI enrichment...")
            try:
                ...
                logger.info(f"[Step 7/25] Done.\n")
            except Exception as e:
                logger.error(f"[Step 7/25] FAILED: {e}")
    ...
    if args.skip_entity_resolution:
        logger.info("[Step 8/25] SKIPPED (--skip-entity-resolution)\n")
    elif not master_ready:
        logger.info("[Step 8/25] SKIPPED — no master data yet\n")
    else:
        logger.info("[Step 8/25] Resolving top 100 vendor entities...")
        try:
            from scripts.entity_resolution import run as run_entity
            run_entity(root=root)
            logger.info("[Step 8/25] Done.\n")
        except Exception as e:
            logger.error(f"[Step 8/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 9: Dominance analysis
    # ------------------------------------------------------------------
    if args.skip_dominance:
        logger.info("[Step 9/25] SKIPPED (--skip-dominance)\n")
    elif not master_ready:
        logger.info("[Step 9/25] SKIPPED — no master data yet\n")
    else:
        logger.info("[Step 9/25] Computing dominance metrics...")
        try:
            from scripts.dominance_analysis import run as run_dominance
            summary_d = run_dominance(root=root)
            logger.info(
                f"[Step 9/25] Done — top vendor: {summary_d.get('top_vendor', '?')}, "
                f"${summary_d.get('top_vendor_obligation', 0):,.0f}\n"
            )
        except Exception as e:
            logger.error(f"[Step 9/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 10: Network graph
    # ------------------------------------------------------------------
    if args.skip_graph:
        logger.info("[Step 10/25] SKIPPED (--skip-graph)\n")
    elif not master_ready:
        logger.info("[Step 10/25] SKIPPED — no master data yet\n")
    else:
        logger.info("[Step 10/25] Building network graph...")
        try:
            from scripts.network_graph import run as run_graph
            summary_g = run_graph(root=root)
            logger.info(
                f"[Step 10/25] Done — {summary_g.get('total_nodes', 0)} nodes, "
                f"{summary_g.get('total_edges', 0)} edges → network.graphml\n"
            )
        except ImportError:
            logger.info("[Step 10/25] SKIPPED — networkx not installed (pip install networkx)\n")
        except Exception as e:
            logger.error(f"[Step 10/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 11: Download grants
    # ------------------------------------------------------------------
    if args.skip_grants:
        logger.info("[Step 11/25] SKIPPED (--skip-grants)\n")
    else:
        logger.info("[Step 11/25] Downloading federal grants (USASpending)...")
        try:
            from scripts.download_grants import run as run_grants
            g = run_grants(root=root)
            logger.info(f"[Step 11/25] Done — {g.get('total_rows', 0):,} grant records\n")
        except Exception as e:
            logger.error(f"[Step 11/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 12: Download subawards
    # ------------------------------------------------------------------
    if args.skip_subawards:
        logger.info("[Step 12/25] SKIPPED (--skip-subawards)\n")
    else:
        logger.info("[Step 12/25] Downloading subawards (USASpending)...")
        try:
            from scripts.download_subawards import run as run_subawards
            s = run_subawards(root=root)
            logger.info(f"[Step 12/25] Done — {s.get('total_rows', 0):,} subaward records\n")
        except Exception as e:
            logger.error(f"[Step 12/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 13: Download FEMA data
    # ------------------------------------------------------------------
    if args.skip_fema:
        logger.info("[Step 13/25] SKIPPED (--skip-fema)\n")
    else:
        logger.info("[Step 13/25] Downloading FEMA Public Assistance + HMGP...")
        try:
            from scripts.download_fema import run as run_fema
            f = run_fema(root=root)
            logger.info(f"[Step 13/25] Done — PA: {f.get('pa_rows', 0):,}, HMGP: {f.get('hmgp_rows', 0):,}\n")
        except Exception as e:
            logger.error(f"[Step 13/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 14: Download research grants
    # ------------------------------------------------------------------
    if args.skip_research:
        logger.info("[Step 14/25] SKIPPED (--skip-research)\n")
    else:
        logger.info("[Step 14/25] Downloading NIH + NSF research grants...")
        try:
            from scripts.download_research import run as run_research
            r = run_research(root=root)
            logger.info(f"[Step 14/25] Done — NIH: {r.get('nih_rows', 0):,}, NSF: {r.get('nsf_rows', 0):,}\n")
        except Exception as e:
            logger.error(f"[Step 14/25] FAILED: {e}")
    ...
    if args.skip_bulk_downloads:
        ...
        logger.info("[Step 15/25] Done.\n")

    # ------------------------------------------------------------------
    # Step 16: Build unified master
    # ------------------------------------------------------------------
    if args.skip_unified_master:
        logger.info("[Step 16/25] SKIPPED (--skip-unified-master)\n")
    else:
        logger.info("[Step 16/25] Building unified awards master (all datasets)...")
        try:
            from scripts.build_unified_master import run as run_unified
            u = run_unified(root=root)
            logger.info(
                f"[Step 16/25] Done — {u.get('total_rows', 0):,} total rows across "
                f"{len(u.get('by_dataset', {}))} datasets → pr_all_awards_master.csv\n"
            )
        except Exception as e:
            logger.error(f"[Step 16/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 17: Download FEC contributions
    # ------------------------------------------------------------------
    if args.skip_fec:
        logger.info("[Step 17/25] SKIPPED (--skip-fec)\n")
    else:
        logger.info("[Step 17/25] Downloading FEC Schedule A contributions from PR...")
        try:
            from scripts.download_fec import run as run_fec
            fec_result = run_fec(root=root, api_key=args.fec_api_key)
            logger.info(f"[Step 17/25] Done — {fec_result.get('rows', 0):,} contribution records\n")
        except Exception as e:
            logger.error(f"[Step 17/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 18: Download LDA lobbying filings
    # ------------------------------------------------------------------
    if args.skip_lda:
        logger.info("[Step 18/25] SKIPPED (--skip-lda)\n")
    else:
        logger.info("[Step 18/25] Downloading LDA lobbying filings for PR...")
        try:
            from scripts.download_lda import run as run_lda
            lda_result = run_lda(root=root, api_key=args.lda_api_key)
            logger.info(f"[Step 18/25] Done — {lda_result.get('rows', 0):,} filings\n")
        except Exception as e:
            logger.error(f"[Step 18/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 19: Cross-reference analyses (FEC + lobbying)
    # ------------------------------------------------------------------
    if args.skip_crossref:
        logger.info("[Step 19/25] SKIPPED (--skip-crossref)\n")
    else:
        logger.info("[Step 19/25] Running FEC and lobbying cross-reference analyses...")
        for label, mod, fn in [
        ...
        logger.info("[Step 19/25] Done.\n")

    # ------------------------------------------------------------------
    # Step 20: IRS 990 nonprofits via ProPublica
    # ------------------------------------------------------------------
    if args.skip_nonprofits:
        logger.info("[Step 20/25] SKIPPED (--skip-nonprofits)\n")
    else:
        logger.info("[Step 20/25] Downloading IRS 990 nonprofit data for PR...")
        try:
            from scripts.download_nonprofits import run as run_nonprofits
            np_result = run_nonprofits(root=root)
            logger.info(f"[Step 20/25] Done — {np_result.get('rows', 0):,} nonprofits\n")
        except Exception as e:
            logger.error(f"[Step 20/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 21: CMS Open Payments + Medicare provider data
    # ------------------------------------------------------------------
    if args.skip_cms:
        logger.info("[Step 21/25] SKIPPED (--skip-cms)\n")
    else:
        logger.info("[Step 21/25] Downloading CMS Open Payments and Medicare data for PR...")
        try:
            from scripts.download_cms import run as run_cms
            cms_result = run_cms(root=root)
            logger.info(
                f"[Step 21/25] Done — Open Payments: {cms_result.get('open_payments_rows', 0):,}, "
                f"Medicare: {cms_result.get('medicare_rows', 0):,}\n"
            )
        except Exception as e:
            logger.error(f"[Step 21/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 22: FDIC bank data
    # ------------------------------------------------------------------
    if args.skip_fdic:
        logger.info("[Step 22/25] SKIPPED (--skip-fdic)\n")
    else:
        logger.info("[Step 22/25] Downloading FDIC bank institution and financial data for PR...")
        try:
            from scripts.download_fdic import run as run_fdic
            fdic_result = run_fdic(root=root)
            logger.info(
                f"[Step 22/25] Done — {fdic_result.get('institution_rows', 0):,} institutions, "
                f"{fdic_result.get('financial_rows', 0):,} financial rows\n"
            )
        except Exception as e:
            logger.error(f"[Step 22/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 23: Entity profiles cross-reference
    # ------------------------------------------------------------------
    if args.skip_entity_profiles:
        logger.info("[Step 23/25] SKIPPED (--skip-entity-profiles)\n")
    else:
        logger.info("[Step 23/25] Building enriched entity profiles...")
        try:
            from scripts.analyze_entity_profiles import build_profiles
            ep_result = build_profiles(root=root)
            logger.info(
                f"[Step 23/25] Done — {ep_result.get('rows', 0):,} entities profiled, "
            ...
            logger.error(f"[Step 23/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 24: SEC EDGAR PR-significant company financials
    # ------------------------------------------------------------------
    if args.skip_sec:
        logger.info("[Step 24/25] SKIPPED (--skip-sec)\n")
    else:
        logger.info("[Step 24/25] Downloading SEC EDGAR data for PR-significant companies...")
        try:
            from scripts.download_sec import run as run_sec
            sec_result = run_sec(root=root)
            logger.info(
                f"[Step 24/25] Done — {sec_result.get('company_rows', 0):,} companies, "
                f"{sec_result.get('financial_rows', 0):,} annual financial rows\n"
            )
        except Exception as e:
            logger.error(f"[Step 24/25] FAILED: {e}")

    # ------------------------------------------------------------------
    # Step 25: Integrated power network analysis
    # ------------------------------------------------------------------
    if args.skip_power_network:
        logger.info("[Step 25/25] SKIPPED (--skip-power-network)\n")
    else:
        logger.info("[Step 25/25] Building integrated PR power/influence network...")
        try:
            from scripts.analyze_power_network import build_power_network
            pn_result = build_power_network(root=root)
            logger.info(
                f"[Step 25/25] Done — {pn_result.get('rows', 0):,} entities ranked, "
                f"{pn_result.get('full_loop', 0):,} full-loop entities "
                f"(awards + FEC + lobbying)\n"
            )
        except Exception as e:
            logger.error(f"[Step 25/25] FAILED: {e}")
