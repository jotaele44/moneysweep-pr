...
SEED_OFFICERS = [
    {
        ...
        "officer_name": "Alvarez Ignacio",
        "officer_position": "President and CEO",
        "is_director": "1",
    ...
    },
    {
        ...
        "officer_name": "Fernandez Jose Rafael",
        "officer_position": "CEO and Vice Chairman",
        "is_director": "1",
...
def _officers_for_company(session, cik: str, name: str, logger) -> list[dict]:
    ...
    logger.info(
        "  %s (CIK %s): %d officers from %d insider filings", company, cik, len(seen), scanned
    )
...
def run(root: Path | None = None, force: bool = False) -> dict:
    ...
    rows: list[dict] = []
    logger.info(
        "Extracting officers from EDGAR insider filings for %d PR-domiciled cos...", len(PR_FILERS)
    )
    for co in PR_FILERS:
