"""
Download Department of Energy grants to/in Puerto Rico from USASpending.

Covers: grid modernization, energy efficiency, weatherization, renewable energy,
and post-Hurricane Maria infrastructure grants. DOE contracts are already
captured by download_contracts.py.
...
Output:
  ...
  data/staging/raw/doe/doe_recipient_<label>.csv
  data/staging/processed/pr_doe_master.csv

Usage:
  python3 scripts/download_doe.py
  python3 scripts/download_doe.py --force
  python3 scripts/download_doe.py --fy-start 2017
"""
...
def build_master(raw_dir: Path, master_path: Path, logger) -> int:
    files = sorted(raw_dir.glob("doe_*.csv"))
    if not files:
        logger.warning("  No raw DOE files found — master not written")
        return 0
    frames = []
    for f in files:
        try:
...
def _run(root: Path = None, force: bool = False, fy_start: int = None) -> dict:
    ...
    raw_dir = root / "data" / "staging" / "raw" / "doe"
    master_path = root / "data" / "staging" / "processed" / "pr_doe_master.csv"
    raw_dir.mkdir(parents=True, exist_ok=True)

    logger = setup_logging("download_doe")
    logger.info("Starting DOE grants download for Puerto Rico...")
    ...
    logger.info("Building DOE master...")
    ...
    logger.info("=" * 60)
    logger.info("DOE DOWNLOAD SUMMARY")
    logger.info("=" * 60)
