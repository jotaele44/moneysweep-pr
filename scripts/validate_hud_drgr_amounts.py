...
def run(root=None, force=False):
        ...
        if "grant_number" in df_activities.columns and "grant_number" in df_projects.columns:
            act_budgets = (
                ...
                .apply(
                    lambda g: (
                        pd.to_numeric(
                            g.get("total_budget", pd.Series(dtype=float)), errors="coerce"
                        )
                        .fillna(0)
                        .sum()
                    )
        ...
        if "activity_id" in df_activities.columns and "activity_id" in df_drawdowns.columns:
            dd_sums = (
                ...
                .apply(
                    lambda g: (
                        pd.to_numeric(
                            g.get("drawdown_amount", pd.Series(dtype=float)), errors="coerce"
                        )
                        .fillna(0)
                        .sum()
                    )
            ...
            for _, act in df_activities.iterrows():
                ...
                if flag == "OK":
                    continue
                note = (
                    f"Drawdown sum (${computed:,.0f}) vs activity amount_drawn (${reported:,.0f})"
                )
                rows.append(
                    {
                        "grant_number": gn,
    ...
    if not df_projects.empty and "disbursement_rate" in df_projects.columns:
        for _, proj in df_projects.iterrows():
            ...
            if rate > 1.0 or (
                rate < 0.01
                and float(pd.to_numeric(proj.get("grant_amount", 0), errors="coerce") or 0)
                > 1_000_000
            ):
                gn = str(proj.get("grant_number", "")).strip()
                ...
                rows.append(
                    {
                        ...
                        "budget_reported": float(
                            pd.to_numeric(proj.get("grant_amount", 0), errors="coerce") or 0
                        ),
                        "budget_computed": float(
                            pd.to_numeric(proj.get("amount_drawn", 0), errors="coerce") or 0
                        ),
    ...
    df_out = (
        pd.DataFrame(rows, columns=RECONCILIATION_COLUMNS)
        if rows
    ...
    logger.info(
        f"  Amount reconciliation: {len(df_out):,} checks, {flagged} flagged ({flag_pct:.1f}%)"
    )
