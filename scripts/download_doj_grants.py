"""
Download Department of Justice grants to/in Puerto Rico from USASpending.

Covers Byrne JAG, COPS, community violence intervention, victim services,
and all other DOJ assistance programs in Puerto Rico.

Output:
  data/staging/raw/doj/doj_pop_<label>.csv
  data/staging/raw/doj/doj_recipient_<label>.csv
  data/staging/processed/pr_doj_grants_master.csv

Usage:
  python3 scripts/download_doj_grants.py
  python3 scripts/download_doj_grants.py --force
  python3 scripts/download_doj_grants.py --fy-start 2017
"""
...
def build_master(raw_dir, master_path, logger):
    files = sorted(raw_dir.glob("doj_*.csv"))
    if not files:
        logger.warning("  No raw DOJ files found — master not written")
        return 0
    frames = []
    for f in files:
        try:
...
def _run(root=None, force=False, fy_start=None):
    if root is None:
        root = PROJECT_ROOT
    raw_dir = root / "data" / "staging" / "raw" / "doj"
    master_path = root / "data" / "staging" / "processed" / "pr_doj_grants_master.csv"
    raw_dir.mkdir(parents=True, exist_ok=True)
    logger = setup_logging("download_doj_grants")
    logger.info("Starting DOJ grants download for Puerto Rico...")
    ...
    for window in windows:
        ...
        logger.info("")
    session.close()
    logger.info("Building DOJ grants master...")
    ...
    logger.info("=" * 60)
    logger.info("DOJ GRANTS DOWNLOAD SUMMARY")
    logger.info("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(description="Download DOJ grants for Puerto Rico")
    parser.add_argument("--force", action="store_true")
    parser.add_argument("--fy-start", type=int, metavar="YEAR")
    args = parser.parse_args()
    summary = _run(force=args.force, fy_start=args.fy_start)
    print(f"\nDOJ grants download complete: {summary['master_rows']:,} master rows")
