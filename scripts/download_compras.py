...
def _fetch_json_endpoint(
    session, endpoint: str, record_type: str, max_pages: int, logger
) -> list[dict]:
    url = COMPRAS_BASE + endpoint
    all_records = []
    for page in range(1, max_pages + 1):
        params = {"page": page, "per_page": PAGE_SIZE, "pageSize": PAGE_SIZE, "limit": PAGE_SIZE}
...
def run(root: Path = None, force: bool = False, max_pages: int = DEFAULT_MAX_PAGES) -> dict:
    ...
    if rfp_out.exists() and award_out.exists() and not force:
        ...
        logger.info(
            f"  Compras: {rfp_rows:,} RFPs, {award_rows:,} awards — skipping (use --force)."
        )
    ...
    award_rows = _write(
        ...
        AWARD_COLUMNS,
        award_out,
        raw_dir / "compras_awards_raw.json",
        "awards",
    )
