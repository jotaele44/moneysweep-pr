"""
Download DOL WIOA (Workforce Innovation and Opportunity Act) grants to Puerto Rico.

WIOA formula grants fund job training, adult education, and youth workforce
development programs. PR receives ~$100-200M/year across three titles:
  - Title I Adult (CFDA 17.258)
  - Title I Dislocated Worker (CFDA 17.259)
  - Title I Youth (CFDA 17.278)

Distinct from DOL enforcement data (download_dol.py); this is the investment
side of DOL's PR presence.

Output:
  data/staging/raw/wioa/wioa_pop_<label>.csv
  data/staging/raw/wioa/wioa_recipient_<label>.csv
  data/staging/processed/pr_wioa_grants.csv

Usage:
  python3 scripts/download_wioa.py
  python3 scripts/download_wioa.py --force
"""
...
def download_window(session, window, raw_dir, force, logger):
    ...
    for filter_type in ("pop", "recipient"):
        fname = f"wioa_{filter_type}_{label}.csv"
        fpath = raw_dir / fname
...
def build_master(raw_dir, master_path, logger):
    files = sorted(raw_dir.glob("wioa_*.csv"))
    if not files:
        logger.warning("  No raw WIOA files found — master not written")
        return 0
    frames = []
    for f in files:
        try:
...
def _run(root=None, force=False, fy_start=None):
    if root is None:
        root = PROJECT_ROOT
    raw_dir = root / "data" / "staging" / "raw" / "wioa"
    master_path = root / "data" / "staging" / "processed" / "pr_wioa_grants.csv"
    raw_dir.mkdir(parents=True, exist_ok=True)
    logger = setup_logging("download_wioa")
    logger.info("Starting WIOA grants download for Puerto Rico...")
...
def main():
    ...
    print(f"\nWIOA download complete: {summary['master_rows']:,} master rows")
