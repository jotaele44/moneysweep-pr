"""
Download Department of Education grants to/in Puerto Rico from USASpending.

Covers Title I, Pell Grants, IDEA special education, TRIO, Head Start (ED share),
and all other ED assistance programs in Puerto Rico.

Output:
  data/staging/raw/ed/ed_pop_<label>.csv
  data/staging/raw/ed/ed_recipient_<label>.csv
  data/staging/processed/pr_ed_master.csv

Usage:
  python3 scripts/download_ed.py
  python3 scripts/download_ed.py --force
  python3 scripts/download_ed.py --fy-start 2017
"""
...
def download_window(session, window, raw_dir, force, logger):
    ...
    for filter_type in ("pop", "recipient"):
        fname = f"ed_{filter_type}_{label}.csv"
        fpath = raw_dir / fname
...
def build_master(raw_dir, master_path, logger):
    files = sorted(raw_dir.glob("ed_*.csv"))
    if not files:
        logger.warning("  No raw ED files found — master not written")
        return 0
    frames = []
    for f in files:
        try:
...
def _run(root=None, force=False, fy_start=None):
    if root is None:
        root = PROJECT_ROOT
    raw_dir = root / "data" / "staging" / "raw" / "ed"
    master_path = root / "data" / "staging" / "processed" / "pr_ed_master.csv"
    raw_dir.mkdir(parents=True, exist_ok=True)
    logger = setup_logging("download_ed")
    logger.info("Starting Department of Education grants download for Puerto Rico...")
    ...
    for window in windows:
        ...
        logger.info("")
    session.close()
    logger.info("Building ED master...")
    ...
    logger.info("=" * 60)
    logger.info("ED DOWNLOAD SUMMARY")
    logger.info("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(description="Download Department of Education grants for Puerto Rico")
    parser.add_argument("--force", action="store_true")
    parser.add_argument("--fy-start", type=int, metavar="YEAR")
    args = parser.parse_args()
    summary = _run(force=args.force, fy_start=args.fy_start)
    print(f"\nED download complete: {summary['master_rows']:,} master rows")
