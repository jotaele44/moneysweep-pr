"""
Download EPA (Environmental Protection Agency) grants and cooperative agreements
to/in Puerto Rico from USASpending.

Covers Superfund cleanup, water/wastewater infrastructure grants, environmental
justice grants, and technical assistance cooperative agreements. PR has 18
Superfund sites; EPA Regions 2 and 4 both have PR jurisdiction.

Output:
  data/staging/raw/epa/epa_pop_<label>.csv
  data/staging/raw/epa/epa_recipient_<label>.csv
  data/staging/processed/pr_epa_master.csv

Usage:
  python3 scripts/download_epa.py
  python3 scripts/download_epa.py --force
  python3 scripts/download_epa.py --fy-start 2017
"""
...
def _run(root=None, force=False, fy_start=None):
    if root is None:
        root = PROJECT_ROOT
    raw_dir = root / "data" / "staging" / "raw" / "epa"
    master_path = root / "data" / "staging" / "processed" / "pr_epa_master.csv"
    raw_dir.mkdir(parents=True, exist_ok=True)
    logger = setup_logging("download_epa")
    logger.info("Starting EPA grants download for Puerto Rico...")
    ...
    logger.info("=" * 60)
    logger.info("EPA DOWNLOAD SUMMARY")
...
def main():
    parser = argparse.ArgumentParser(description="Download EPA grants for Puerto Rico")
    parser.add_argument("--force", action="store_true")
    parser.add_argument("--fy-start", type=int, metavar="YEAR")
    args = parser.parse_args()
    summary = _run(force=args.force, fy_start=args.fy_start)
    print(f"\nEPA download complete: {summary['master_rows']:,} master rows")
