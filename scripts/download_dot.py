"""
Download Department of Transportation grants to/in Puerto Rico from USASpending.

Covers: FHWA formula grants, FTA transit grants, FAA airport grants, and other
DOT assistance awards. DOT contracts are already captured by download_contracts.py.
...
Output:
  data/staging/raw/dot/dot_pop_<label>.csv
  data/staging/raw/dot/dot_recipient_<label>.csv
  data/staging/processed/pr_dot_master.csv

Usage:
  python3 scripts/download_dot.py
...
def download_window(session, window, raw_dir, force, logger) -> dict:
    ...
    for filter_type in ("pop", "recipient"):
        fname = f"dot_{filter_type}_{label}.csv"
        fpath = raw_dir / fname
...
def build_master(raw_dir: Path, master_path: Path, logger) -> int:
    files = sorted(raw_dir.glob("dot_*.csv"))
    if not files:
        logger.warning("  No raw DOT files found — master not written")
        return 0
    frames = []
    for f in files:
        try:
...
def _run(root: Path = None, force: bool = False, fy_start: int = None) -> dict:
    ...
    raw_dir = root / "data" / "staging" / "raw" / "dot"
    master_path = root / "data" / "staging" / "processed" / "pr_dot_master.csv"
    raw_dir.mkdir(parents=True, exist_ok=True)
    ...
    logger.info("Building DOT master...")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Download DOT grants for Puerto Rico")
