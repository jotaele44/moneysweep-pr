...
def inventory_processed_files(root: Path) -> dict[str, Any]:
    ...
    for p in sorted(proc.glob("*.csv")):
        ...
        files.append({
            ...
            "classification": "declared" if claimed_by else ("orphan" if is_orphan else "empty"),
        })
...
def compute_local_coverage(root: Path) -> dict[str, Any]:
    ...
    summary["coverage_rate_fully"] = round(
        summary["fully_materialized"] / summary["total_sources"], 4
    ) if summary["total_sources"] else 0.0
...
def probe_usaspending_prime(session, logger) -> dict[str, Any]:
    ...
    data = http_post_json(session, USASPENDING_COUNT_URL, payload,
                          logger=logger, config=_PROBE_HTTP)
    ...
    return {
        ...
        "assistance_universe": (
            int(r.get("grants", 0)) + int(r.get("direct_payments", 0))
            + int(r.get("loans", 0)) + int(r.get("other", 0))
        ),
    }
...
def probe_fema_pa(session, logger) -> dict[str, Any]:
    ...
    skip = 0
    while True:
        url = (f"{FEMA_BASE_V2}DisasterDeclarationsSummaries"
    ...
    for i in range(0, len(ordered), batch_size):
        ...
        url = (f"{FEMA_BASE_V2}PublicAssistanceFundedProjectsDetails"
...
def run_universe_probes(root: Path, local: dict[str, Any],
    ...
    if us.get("status") == "ok":
        ...
        crow = _uni_row(
            "usaspending_prime", "contracts (+IDVs), FY2008+", c_fy2008, c_univ,
            "USAspending spending_by_award_count", us["scope"],
            note=(f"numerator = FY2008+ records only; master also holds "
        ...
        grow = _uni_row(
            "usaspending_prime_grants", "all assistance (grants+dp+loans+other)",
            g_records, us["assistance_universe"], "USAspending spending_by_award_count",
            us["scope"],
            note=("orphan file pr_grants_master.csv (no registry source); APPROXIMATE — "
    ...
    if fema.get("status") == "ok":
        results.append(_uni_row(
            "fema_pa_openfema_v2", "PA project details", rows("fema_pa_openfema_v2"),
            fema["universe"], "OpenFEMA $inlinecount", fema["scope"],
        ))
    else:
    ...
    if lda.get("status") == "ok":
        results.append(_uni_row(
            "lda", "federal lobbying filings", rows("lda"),
            lda["universe"], "lda.senate.gov filings count", lda["scope"],
        ))
    else:
    ...
    for sid, label, disk_file, note in [
        ("fec", "campaign contributions", "pr_fec_contributions.csv",
         "key-gated (FEC_API_KEY); denominator is an estimate"),
        ("sam_entities", "registered entities", "",
         "key-gated (SAM_API_KEY); not materialized locally"),
        ("emma_bonds", "municipal bond issues", "pr_emma_bonds.csv",
         "EMMA has no keyless count API"),
        ("ofac_sdn", "SDN list (global, not PR-scoped)", "pr_ofac_sdn.csv",
...
def write_outputs(root: Path, audit: dict[str, Any]) -> tuple[Path, Path]:
    ...
    fields = [
        "source_id", "family", "required", "authentication",
        "expected_output_count", "present_count", "local_rows",
        "local_status", "materiality_tier", "first_expected_output",
    ]
    ...
    with files_csv.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["file", "rows", "classification", "claimed_by"],
                          extrasaction="ignore")
        w.writeheader()
