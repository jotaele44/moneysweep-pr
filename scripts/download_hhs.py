"""
Download HHS (HRSA + ACF) grants to/in Puerto Rico from USASpending.

Covers Health Resources and Services Administration (community health centers,
rural health, maternal/child health) and Administration for Children and Families
(Head Start, TANF, CCDF, child welfare).

Output:
  data/staging/raw/hhs/hhs_pop_<label>.csv
  data/staging/raw/hhs/hhs_recipient_<label>.csv
  data/staging/processed/pr_hhs_master.csv

Usage:
  python3 scripts/download_hhs.py
  python3 scripts/download_hhs.py --force
  python3 scripts/download_hhs.py --fy-start 2017
"""
...
def _build_payload(filter_type, window, subtier=None):
    ...
    if subtier:
        agencies.append({"type": "awarding", "tier": "subtier", "name": subtier})
    return {
        "filters": {
...
def download_window(session, window, raw_dir, force, logger):
    label = window["label"]
    stats = {"window": label, "pop_rows": 0, "recipient_rows": 0, "errors": []}
    # Fetch once per subtier per filter_type to avoid result truncation
    for subtier in SUBTIER_AGENCIES:
        subtier_slug = subtier.lower().replace(" ", "_")[:10]
        for filter_type in ("pop", "recipient"):
            fname = f"hhs_{subtier_slug}_{filter_type}_{label}.csv"
            fpath = raw_dir / fname
            if not force and _file_has_data(fpath):
                ...
                continue
            logger.info(f"  Fetching {fname} ({subtier}, filter={filter_type})")
            results = _paginate(session, _build_payload(filter_type, window, subtier), logger)
            if not results:
...
def build_master(raw_dir, master_path, logger):
    files = sorted(raw_dir.glob("hhs_*.csv"))
    if not files:
        logger.warning("  No raw HHS files found — master not written")
        return 0
    frames = []
    for f in files:
        try:
...
def _run(root=None, force=False, fy_start=None):
    if root is None:
        root = PROJECT_ROOT
    raw_dir = root / "data" / "staging" / "raw" / "hhs"
    master_path = root / "data" / "staging" / "processed" / "pr_hhs_master.csv"
    raw_dir.mkdir(parents=True, exist_ok=True)
    logger = setup_logging("download_hhs")
    logger.info("Starting HHS (HRSA + ACF) grants download for Puerto Rico...")
...
def main():
    parser = argparse.ArgumentParser(description="Download HHS grants for Puerto Rico")
    parser.add_argument("--force", action="store_true")
    parser.add_argument("--fy-start", type=int, metavar="YEAR")
    args = parser.parse_args()
    summary = _run(force=args.force, fy_start=args.fy_start)
    print(f"\nHHS download complete: {summary['master_rows']:,} master rows")
