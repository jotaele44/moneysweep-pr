...
def _extract_annual_series(
    facts: dict, concept_names: list[str], unit_filter: str = "USD"
...
def _company_financials(cik: str, ticker: str, name: str, facts: dict) -> list[dict]:
    ...
    for field, concepts in XBRL_CONCEPTS.items():
        unit = (
            "pure"
            if field in ("shares_outstanding",)
            else "any"
...
def run(root: Path = None, force: bool = False) -> dict:
    ...
    for i, co in enumerate(all_companies, 1):
        ...
        if facts:
            ...
            emp_series = _extract_annual_series(
                facts, ["EntityNumberOfEmployees"], unit_filter="any"
            )
            if emp_series:
        ...
        fin_rows = [
            {
                ...
                "name": "Popular Inc",
                "fiscal_year": "2022",
                "total_revenues": "2800000000",
                "net_income": "620000000",
                "total_assets": "72000000000",
                "total_liabilities": "65000000000",
                "stockholders_equity": "7000000000",
            ...
            },
            {
                ...
                "name": "First BanCorp",
                "fiscal_year": "2022",
                "total_revenues": "650000000",
                "net_income": "185000000",
                "total_assets": "14000000000",
                "total_liabilities": "12600000000",
                "stockholders_equity": "1400000000",
            ...
            },
            {
                ...
                "fiscal_year": "2022",
                "total_revenues": "1200000000",
                "net_income": "85000000",
                "total_assets": "1800000000",
                "total_liabilities": "1400000000",
                "stockholders_equity": "400000000",
    ...
    if not df_fin.empty:
        df_latest = (
            df_fin.sort_values("fiscal_year", ascending=False)
            .drop_duplicates("cik")
            .sort_values(
                "total_revenues",
                ascending=False,
        ...
        for _, row in df_latest.head(10).iterrows():
            ...
            logger.info(
                f"    {str(row.get('name', ''))[:50]:<50} {rev_str}  [{sector}]  FY{row.get('fiscal_year', '')}"
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download SEC EDGAR data for PR-significant companies"
    )
