"""
Shared dropzone-reader for infrastructure CONTRACT producers (outflow side).

Generalizes the ``scripts/ingest_prasa.py`` pattern so the new infrastructure
contract families (DTOP/ACT permanent roads, Ports & Airports, transit O&M) reuse
one normalizer instead of copy-pasting the column map. Dropzone reader, not a scraper.

Output schema (same shape as pr_prasa_contracts.csv):

    contract_id, vendor_name, vendor_normalized, contract_type, contract_value,
    award_date, start_date, end_date, status, description, municipality,
    agency, source_file
"""
...
COL_MAP = {
    ...
    "start_date": ["Start Date", "Fecha de Inicio", "Begin Date", "Inicio", "fecha_inicio"],
    "end_date": [
        "End Date",
    ...
    "status": ["Status", "Estado", "Estatus", "Active", "Activo", "Contract Status", "estado"],
    "description": [
        "Description",
        "Descripción",
        "Scope",
    ...
    "municipality": ["Municipality", "Municipio", "Location", "Lugar", "municipio"],
}
...
def _parse_df(df, source_file, agency, logger):
    if df.empty:
    ...
    logger.info(f"  → {len(result):,} contract records from {source_file}")
...
def run_contract_ingest(*, logger_name, drop_subdir, out_filename, agency, root=None, force=False):
    ...
    logger.info(f"Starting {agency} contract ingestion...")
    ...
    if not files:
        logger.warning(f"  No files found. Place contract exports in {drop_subdir}")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        pd.DataFrame(columns=CONTRACT_COLUMNS).to_csv(out_path, index=False, encoding="utf-8")
    ...
    for f in files:
        ...
        df_out = _parse_df(df_raw, f.name, agency, logger)
        if not df_out.empty:
    ...
    logger.info("=" * 60)
    logger.info(f"{agency} CONTRACTS SUMMARY")
    logger.info("=" * 60)
