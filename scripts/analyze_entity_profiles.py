...
def build_profiles(root: Path | None = None) -> dict:
    ...
    if df_990 is not None and not df_990.empty:
        ...
        np_unmatched["_size"] = pd.to_numeric(
            np_unmatched["np_total_revenue"], errors="coerce"
        ).fillna(0)
        gap_rows.append(
            np_unmatched[["_norm", "_source", "_size", "np_name", "np_ntee_category"]].rename(
                columns={
                    "_norm": "normalized_name",
                    "np_name": "entity_name",
    ...
    if df_med is not None and not df_med.empty:
        ...
        med_unmatched["_size"] = pd.to_numeric(
            med_unmatched["med_total_payment"], errors="coerce"
        ).fillna(0)
        gap_rows.append(
            med_unmatched[
                ["_norm", "_source", "_size", "med_provider_name", "med_provider_type"]
            ].rename(
                columns={
                    "_norm": "normalized_name",
                    "med_provider_name": "entity_name",
    ...
    if df_banks is not None and not df_banks.empty:
        ...
        bank_unmatched = bank_index[
            ~bank_index["_norm"].isin(entity_spine["normalized_name"])
        ].copy()
        bank_unmatched["_source"] = "fdic_bank"
        bank_unmatched["_size"] = pd.to_numeric(
            bank_unmatched["bank_total_assets"], errors="coerce"
        ).fillna(0)
        gap_rows.append(
            bank_unmatched[["_norm", "_source", "_size", "bank_name"]]
            .rename(columns={"_norm": "normalized_name", "bank_name": "entity_name"})
            .assign(entity_category="bank")
        )
    else:
    ...
    if not np_index.empty and "_norm" in np_index.columns:
        merged = merged.merge(
            np_index.rename(columns={"_norm": "normalized_name"}), on="normalized_name", how="left"
        )
    if not med_index.empty and "_norm" in med_index.columns:
        merged = merged.merge(
            med_index.rename(columns={"_norm": "normalized_name"}), on="normalized_name", how="left"
        )
    ...
    merged["is_medicare_provider"] = merged.get(
        "med_total_payment", pd.Series(dtype=str)
    ).notna() & (merged.get("med_total_payment", pd.Series(dtype=str)) != "")
    ...
    if gap_rows:
        ...
        pd.DataFrame(
            columns=["normalized_name", "_source", "_size", "entity_name", "entity_category"]
    ...
    multi_match = int(
        ((flag_np.astype(int) + flag_med.astype(int) + flag_bank.astype(int)) >= 2).sum()
    )

    float(merged["total_awards_obligated"].sum())
    np_awards = (
        float(pd.to_numeric(merged.loc[flag_np, "total_awards_obligated"], errors="coerce").sum())
        if np_matched
        else 0
    )
    med_awards = (
        float(pd.to_numeric(merged.loc[flag_med, "total_awards_obligated"], errors="coerce").sum())
        if med_matched
        else 0
    )
    ...
    logger.info(
        f"  Matched as Medicare provider:    {med_matched:,}  (${med_awards:,.0f} in awards)"
    )
