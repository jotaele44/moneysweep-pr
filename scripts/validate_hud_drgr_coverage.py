...
def run(root=None, force=False):
    ...
    logger.info(
        f"  Org resolution: {resolved}/{total_link_orgs} ({resolved_pct:.1f}%) — "
    ...
    if not df_activities.empty:
        ...
        if not df_linkage.empty and "responsible_org_normalized" in df_linkage.columns:
            linked_orgs = set(
                df_linkage[df_linkage["link_confidence"] != "none"][
    ...
    df_unlinked = (
        pd.DataFrame(unlinked_rows) if unlinked_rows else pd.DataFrame(columns=GAP_COLUMNS)
    )
    ...
    logger.info(
        f"  Org resolution:      {resolved_pct:.1f}% — {'PASS' if coverage_pass else 'BELOW THRESHOLD'}"
    )
