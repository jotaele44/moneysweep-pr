...
ENTITY_COLUMNS = [
    "lei", "legal_name", "other_names", "jurisdiction", "legal_form",
    "entity_status", "registered_as", "legal_address", "hq_address",
    "registration_date", "source_url",
]
RELATIONSHIP_COLUMNS = [
    "lei", "legal_name", "relationship_type", "parent_lei", "parent_name",
]
...
def _fetch_entities(session, logger) -> list[dict]:
    ...
    for filt in PR_FILTERS:
        ...
        while page <= MAX_PAGES:
            ...
            logger.info("  %s page %d/%d (+%d, total unique=%d)",
                        list(filt.values())[0], page, last, len(records), len(by_lei))
...
def _fetch_relationships(session, entities: list[dict], logger) -> list[dict]:
    ...
    for i, ent in enumerate(entities, 1):
        lei = ent["lei"]
        for rel_type, slug in (("direct_parent", "direct-parent"),
            ...
            if plei:
                rows.append({
                    ...
                    "parent_lei": plei,
                    "parent_name": (pattrs.get("entity", {}).get("legalName") or {}).get("name", ""),
                })
        if i % 100 == 0:
...
def main() -> int:
    ...
    p.add_argument("--no-relationships", action="store_true", help="Skip parent-relationship calls.")
    a = p.parse_args()
