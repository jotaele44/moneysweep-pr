"""Producer patch retry support for R4.8F."""
...
DETERMINISTIC_PRODUCER_COMMANDS = {
...
def run_producer_patch_retries(
    ...
    *,
    ...
    producer_patches_applied = 0
    producer_retries_attempted = 0
    producer_retries_successful = 0
    ...
    for row in sorted(producer_rows, key=lambda item: safe_int(item.get("priority"))):
        ...
        producer_classification = str(row.get("producer_classification", "")).strip()
        ...
        command = DETERMINISTIC_PRODUCER_COMMANDS.get(producer_script, "")
        if command:
            report["deterministic_patch_applied"] = True
            producer_patches_applied += 1
            report["retry_command"] = command
        else:
            report["retry_status"] = "no_deterministic_patch_available"
            if not report["failure_reason"]:
                report["failure_reason"] = "no deterministic producer patch command"
            report_rows.append(report)
            remaining_rows.append({**report, "review_status": "pending"})
            continue

        report["retry_attempted"] = True
        producer_retries_attempted += 1
        ...
        manifest_row = build_manifest(
            root,
            priority=priority,
            source_system=source_family or "producer_retry_source",
            source_file=target_output_path,
            target_output_path=target_output_path,
            row_count=rows,
            producer_script=producer_script,
            known_gaps="r4_8f_producer_retry",
        )
        new_manifests.append(manifest_row)

        report["manifest_written"] = True
        report["retry_status"] = "success"
        producer_retries_successful += 1
        report_rows.append(report)

    metrics = {
        "producer_failures_reviewed": len(producer_rows),
        "producer_patches_applied": producer_patches_applied,
        "producer_retries_attempted": producer_retries_attempted,
        "producer_retries_successful": producer_retries_successful,
    }
