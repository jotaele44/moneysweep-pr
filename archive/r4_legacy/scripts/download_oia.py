"""
Download Office of Insular Affairs (OIA) grants to/in Puerto Rico from USASpending.

OIA is within the Department of the Interior and administers territorial assistance
grants unique to US territories including Puerto Rico.

Output:
  data/staging/raw/oia/oia_pop_<label>.csv
  data/staging/raw/oia/oia_recipient_<label>.csv
  data/staging/processed/pr_oia_master.csv

Usage:
  python3 scripts/download_oia.py
  python3 scripts/download_oia.py --force
  python3 scripts/download_oia.py --fy-start 2017
"""
...
SUBTIER_AGENCY = "Office of Insular Affairs"
    ...
    return {
        "filters": {
            ...
            "agencies": [
                ...
                {"type": "awarding", "tier": "subtier", "name": SUBTIER_AGENCY},
            ],
...
def download_window(session, window, raw_dir, force, logger):
    ...
    for filter_type in ("pop", "recipient"):
        fname = f"oia_{filter_type}_{label}.csv"
        fpath = raw_dir / fname
...
def build_master(raw_dir, master_path, logger):
    files = sorted(raw_dir.glob("oia_*.csv"))
    if not files:
        logger.warning("  No raw OIA files found — master not written")
        return 0
    frames = []
    for f in files:
        try:
...
def _run(root=None, force=False, fy_start=None):
    if root is None:
        root = PROJECT_ROOT
    raw_dir = root / "data" / "staging" / "raw" / "oia"
    master_path = root / "data" / "staging" / "processed" / "pr_oia_master.csv"
    raw_dir.mkdir(parents=True, exist_ok=True)
    logger = setup_logging("download_oia")
    logger.info("Starting OIA grants download for Puerto Rico...")
    ...
    for window in windows:
        ...
        logger.info("")
    session.close()
    logger.info("Building OIA master...")
    ...
    logger.info("=" * 60)
    logger.info("OIA DOWNLOAD SUMMARY")
    logger.info("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(description="Download OIA grants for Puerto Rico")
    parser.add_argument("--force", action="store_true")
    parser.add_argument("--fy-start", type=int, metavar="YEAR")
    args = parser.parse_args()
    summary = _run(force=args.force, fy_start=args.fy_start)
    print(f"\nOIA download complete: {summary['master_rows']:,} master rows")
