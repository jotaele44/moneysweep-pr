...
| 4 | Runtime utilities | Done (R5 PR1: `contract_sweeper/runtime/` — 8 modules). | Add `geo_runtime`, `assets_taxonomy` in later PR. |
...
| 13 | Validation gates | Done (R5 PR1: `contract_sweeper/runtime/validation_gates.py`, 7+ gates). | Flipped from `--allow-failed` to enforced after PR2 lands real data. |
...
- 9 runtime modules in `contract_sweeper/runtime/`.
