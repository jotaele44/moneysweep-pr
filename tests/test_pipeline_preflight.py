...
def test_api_key_status_reports_present_and_missing(monkeypatch, caplog):
    ...
    # intentionally unconfigured (see .env.example) and never lives in .env, so
    # it stays MISSING regardless of operator .env contents. (FEC is no longer a
    # valid "missing" example now that its key is provisioned in .env.)
