...
from readiness.moneysweep_finance_lane import (  # noqa: E402
    EXPORT_CONTRACT_VERSION,
    FINANCE_FIELDS,
    ContractSweeperFinanceLaneError,
    build_moneysweep_finance_lane,
)

_REPORT_SCHEMA = Path(__file__).resolve().parents[1] / (
    "schemas/moneysweep_finance_lane_report.schema.json"
)
...
def _write(tmp_path, rows):
    p = tmp_path / "moneysweep_derivatives.csv"
...
def test_missing_input_raises(tmp_path):
    with pytest.raises(ContractSweeperFinanceLaneError):
        build_moneysweep_finance_lane(tmp_path)


def test_row_routes_to_named_tables(tmp_path):
    report = build_moneysweep_finance_lane(_write(tmp_path, [_row()]), tmp_path / "out")
    ...
    row = _row(
        ...
        output_tables=json.dumps(
            [
                ...
                "funding_event_leads",
                "moneysweep_crosswalk_queue",
                "verification_queue",
            ]
        ),
    )
    report = build_moneysweep_finance_lane(_write(tmp_path, [row]), tmp_path / "out")
    assert report["by_table"]["contracts_procurement_events"] == 1
    assert report["by_table"]["funding_event_leads"] == 1
    assert report["by_review_queue"]["moneysweep_crosswalk_queue"] == 1
    assert report["by_review_queue"]["verification_queue"] == 1


def test_empty_domains_goes_to_discrepancy(tmp_path):
    report = build_moneysweep_finance_lane(_write(tmp_path, [_row(domains="[]")]), tmp_path / "out")
...
def test_unrecognized_output_tables_goes_to_discrepancy(tmp_path):
    report = build_moneysweep_finance_lane(
...
def test_missing_record_id_goes_to_discrepancy(tmp_path):
    report = build_moneysweep_finance_lane(_write(tmp_path, [_row(record_id="")]), tmp_path / "out")
    assert report["discrepancy_count"] == 1
...
def test_zero_loss_accounting(tmp_path):
    ...
    report = build_moneysweep_finance_lane(_write(tmp_path, rows), tmp_path / "out")
...
def test_finance_records_carry_no_geometry(tmp_path):
    build_moneysweep_finance_lane(_write(tmp_path, [_row()]), tmp_path / "out")
    funding = _read(tmp_path / "out" / "data" / "normalized" / "funding_event_leads.csv")
