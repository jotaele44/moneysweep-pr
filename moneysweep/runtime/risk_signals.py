...
from moneysweep.runtime.source_registry import REPO_ROOT
...
def _signals_concentration(awards: pd.DataFrame) -> list[dict]:
    ...
    col = (
        ...
        if "recipient_name_normalized" in awards.columns
        else "recipient_name"
    )
            ...
            signals.append(
                {
                    "signal_id": _signal_id(
                        "parent_sub_mismatch", row.get("chain_id", str(seq)), seq
                    ),
...
def _signals_geographic_clustering(awards: pd.DataFrame) -> list[dict]:
    ...
    for seq, (place, count) in enumerate(counts.items()):
        ...
        top_recipients = (
            top_rows.get(
                "recipient_name_normalized", top_rows.get("recipient_name", pd.Series(dtype=str))
            )
            .value_counts()
            .head(3)
            .index.tolist()
        ...
        signals.append(
            {
                ...
                "confidence": 0.80,
                "evidence_source": "pr_all_awards_master",
                "evidence_row_ids": ";".join(
                    top_rows.get("award_id", pd.Series(dtype=str)).tolist()[:10]
                ),
                "explanation": (
...
def _signals_stale_lineage(awards: pd.DataFrame) -> list[dict]:
    ...
    for seq, (_, row) in enumerate(awards.iterrows()):
        missing = [f for f in LINEAGE_REQUIRED_FIELDS if not str(row.get(f, "")).strip()]
        if not missing:
            continue
        signals.append(
            {
                ...
                "entity_name": str(
                    row.get("recipient_name_normalized", row.get("recipient_name", ""))
                ),
                ...
                "explanation": (
                    f"Award {row.get('award_id', '')} missing lineage fields: {', '.join(missing)}"
                ),
            }
        )
    return signals
    ...
    if not awards.empty:
        ...
        if col:
            for place, grp in awards.groupby(col):
                ...
                top_r_col = (
                    "recipient_name_normalized"
                    if "recipient_name_normalized" in grp.columns
                    else "recipient_name"
                )
