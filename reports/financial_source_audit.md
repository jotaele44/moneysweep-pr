...
Registry sources: **126** (financial: **119**, supporting/reference: **7**). Not-yet-considered candidates: **3**.
...
| `not_considered` | 3 | 0 | Real-world financial source with no registry entry yet. |

## The four questions

1. **Which financial sources are wired?** 114 are wired to a producer — 2 producing output now, 3 able to materialize fully offline from committed inputs, 60 automatable & ready to run (incl. key-gated), 49 wired but queued behind a manual export or scraper.
...
4. **Which haven't even been considered?** 3 real-world financial sources have no registry entry (see `financial_source_coverage_gaps.md`).
...
| `territorial_spending` | 24 | 0 | 23 |

## Producer/source-id name mismatches (26)
