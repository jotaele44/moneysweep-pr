...
| `wired_not_set_to_materialize` | 8 | 6 | Wired but produces nothing by design (deferred stub / sibling duplicate). |
| `queued_manual` | 33 | 33 | Wired, but waits on an operator-delivered manual export. |
| `queued_scraper` | 18 | 18 | Declared, but needs a scraping adapter for a PR-gov HTML/PDF surface. |
...
1. **Which financial sources are wired?** 116 are wired to a producer — 2 producing output now, 3 able to materialize fully offline from committed inputs, 60 automatable & ready to run (incl. key-gated), 51 wired but queued behind a manual export or scraper.
...
3. **Which aren't set to materialize anything?** 6 produce nothing by design (deferred stubs + semantic duplicates of sibling sources).
...
| `pr_general_fund_revenues` | `download_coverage_gap_intake.py` | territorial_spending |
| `pr_income_tax_collections` | `download_coverage_gap_intake.py` | territorial_spending |
